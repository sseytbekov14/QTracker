...
class RecurringDay5Day12ReminderServiceTest {
    ...
    void day5WithFacilitatorResponseSendsToFacilitatorAndOperatorOnly() {
        ...
        when(notificationRepository.existsByControlIdAndTypeAndCreatedAtGreaterThanEqualAndCreatedAtLessThan(
                eq(51L),
                eq(RecurringNotificationService.TYPE_DAY5),
        ...
        control.setControlFrequency("Recurring");
        control.setControlStatus("IN_PROGRESS");
        assertThat(saved).allSatisfy(notification -> {
            assertThat(notification.getType()).isEqualTo(RecurringNotificationService.TYPE_DAY5);
        ...
        assertThat(saved).extracting(Notification::getMessage).containsOnly(
                ControlNotificationText.dayReminderBody(5, control, operationDate, "/view-control/51")
        );
    ...
    void day12WithoutResponseSendsToFacilitatorAndOperatorOnly() {
        ...
        when(notificationRepository.existsByControlIdAndTypeAndCreatedAtGreaterThanEqualAndCreatedAtLessThan(
                eq(52L),
                eq(RecurringNotificationService.TYPE_DAY12),
                any(LocalDateTime.class),
                any(LocalDateTime.class))).thenReturn(false);
        ...
        assertThat(saved).allSatisfy(notification ->
                assertThat(notification.getType()).isEqualTo(RecurringNotificationService.TYPE_DAY12));
        assertThat(saved).extracting(Notification::getMessage).containsOnly(
                ControlNotificationText.dayReminderBody(12, control, operationDate, "/view-control/52")
        );
        verify(userRepository, never()).findByMail("owner@kpmg.kz");
    }
