...
class AnnualSemiDay5Day25ReminderServiceTest {
    ...
    void day5WithFacilitatorResponseSendsToFacilitatorAndOperatorOnly() {
        ...
        when(notificationRepository.existsByControlIdAndTypeAndCreatedAtGreaterThanEqualAndCreatedAtLessThan(
                eq(101L),
                eq(AnnualNotificationService.TYPE_DAY5),
        ...
        assertThat(saved).allSatisfy(notification -> {
            assertThat(notification.getType()).isEqualTo(AnnualNotificationService.TYPE_DAY5);
        ...
        assertThat(saved).extracting(Notification::getMessage).containsOnly(
                ControlNotificationText.dayReminderBody(5, control, operationDate, "/view-control/101")
        );
        verify(userRepository, never()).findByMail("owner@kpmg.kz");
    }

    @Test
    void day25WithoutResponseSendsToFacilitatorAndOperatorOnly() {
        LocalDate today = LocalDate.now(clock);
        ...
        when(notificationRepository.existsByControlIdAndTypeAndCreatedAtGreaterThanEqualAndCreatedAtLessThan(
                eq(102L),
                eq(AnnualNotificationService.TYPE_DAY25),
                any(LocalDateTime.class),
                any(LocalDateTime.class))).thenReturn(false);
        ...
        assertThat(saved).allSatisfy(notification ->
                assertThat(notification.getType()).isEqualTo(AnnualNotificationService.TYPE_DAY25));
        assertThat(saved).extracting(Notification::getMessage).containsOnly(
                ControlNotificationText.dayReminderBody(25, control, operationDate, "/view-control/102")
        );
        verify(userRepository, never()).findByMail("owner@kpmg.kz");
    }
